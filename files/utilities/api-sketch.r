...
#
#mark<-
#   glyph(left=df1[,1]-min(df1[,1]),bottom=df1[,2]-min(df1[,2]),stroke=NA,fill='black',size=5,parent=plot1)
#mark2<-rect(left=df1[,1]-(0.5*rectW)-min(df1[,1]),bottom=df1[,2]- (0.5*rectH)-
#   min(df1[,2]),height=rectH,
#   width=rectW,stroke=NA,fill=col2rgb(rgb(1,seq(0,1,length=nrow(df1)),0,0.5),T),parent=plot1)
...
#mark5<-vbar(left=50-min(df1[,1]),bottom=c(-33,83)-min(df1[,2]),top=c(0,115)-min(df1[,2]),parent=plot1,
#   stroke='blue')
...
# bottom - the distance from the bottom edge of the parent panel to the dot
#   center.
# right - the distance from the right edge of the parent panel to the dot
#   center.
...
glyph <- function(top = NULL, left = NULL, bottom = NULL, right = NULL, 
    fill = "black", stroke = NA, size = 5, parent = NULL) {
    #error handling
    #need one each of left/right; top/bottom
    if ((is.null(left) & is.null(right)) | (is.null(top) & is.null(bottom)) | (!is.null(left) & 
        !is.null(right)) | (!is.null(top) & !is.null(bottom))) {
        print("incorrect arguments: need one each of left/right and top/bottom")
    }
    structure(list(left = left, right = right, top = top, bottom = bottom, fill = fill, 
        stroke = stroke, size = size, parent = parent), class = c("cranvas", "glyph"))
}
...
#Comments:
## Do we really need to modify arguments to support protovis naming conventions
#   only to modify them again to support qtpaint property conventions?
## Wouldn't it be easier to have qtpaint support property conventions? Note
#   this question does not address superficial naming conventions, rather
## observing that protovis defines the shape using width and height properties.
#   qdrawRect explicitly uses left right top bottom.

rect <- function(left = NULL, right = NULL, top = NULL, bottom = NULL, 
    width = NULL, height = NULL, fill = "black", stroke = NULL, parent = NULL) {
    if ((is.null(left) & is.null(right)) | (is.null(right) & is.null(width)) | (is.null(left) & 
        is.null(width)) | (!is.null(left) & !is.null(right) & !is.null(width)) | 
        (is.null(top) & is.null(bottom)) | (is.null(top) & is.null(height)) | (is.null(bottom) & 
        is.null(height)) | (!is.null(top) & !is.null(bottom) & !is.null(height))) {
        print("need 2 of 3 in *left,right,width* and *top,bottom,height*")
    }
    structure(list(left = left, bottom = bottom, right = right, top = top, height = height, 
        width = width, parent = parent, fill = fill, stroke = stroke), class = c("cranvas", 
        "rect"))
}
...
# minimum properties: left, bottom
# top - the distance from the top edge of the parent panel to the line
#   center.NOTE: see changes below
# left - the distance from the left edge of the parent panel to the line
#   center.NOTE: see changes below
# bottom - the distance from the bottom edge of the parent panel to the line
#   center.NOTE: see changes below
# right - the distance from the right edge of the parent panel to the line
#   center.NOTE: see changes below
# defining the top property instead of bottom, the line is flipped vertically:
# using right instead of left flips horizontally

#Changes/Additions from protovis:
# top,left,bottom,right measure distance to segment endpoints #like specs for
#   provis rule (see below)

line <- function(left = NULL, right = NULL, top = NULL, bottom = NULL, 
    stroke = "black", width = 1, parent = NULL) {
    if ((!is.null(left) & !is.null(right)) | (is.null(left) & is.null(right)) | (!is.null(top) & 
        !is.null(bottom)) | (is.null(top) & is.null(bottom))) {
        print("need one of {top,bottom} and {left,right}")
    }
    structure(list(left = left, right = right, top = top, bottom = bottom, stroke = stroke, 
        width = width, parent = parent), class = c("cranvas", "line"))
}
...
hbar <- function(width = 1, top = NULL, bottom = NULL, left = NULL, right = NULL, 
    stroke = "black", parent = parent) {
    if (!is.null(top) & !is.null(bottom)) {
        print("need only one of {top,bottom}")
    }
    if ((!is.null(left) & is.null(right)) | (is.null(left) & !is.null(left))) {
        print("need both left and right")
    }
    structure(list(left = left, right = right, top = top, bottom = bottom, stroke = stroke, 
        width = width, parent = parent), class = c("cranvas", "hbar"))
}

vbar <- function(left = NULL, right = NULL, top = NULL, bottom = NULL, 
    stroke = "black", width = 1, parent = NULL) {
    if (!is.null(left) & !is.null(right)) {
        print("need only one of {left,right}")
    }
    if ((!is.null(top) & is.null(bottom)) | (is.null(top) & !is.null(bottom))) {
        print("need both top and bottom")
    }
    structure(list(left = left, right = right, top = top, bottom = bottom, stroke = stroke, 
        width = width, parent = parent), class = c("cranvas", "vbar"))
}
...
# left - the distance from the left edge of the parent panel to the text
#   anchor.
# bottom - the distance from the bottom edge of the parent panel to the text
#   anchor.
# right - the distance from the right edge of the parent panel to the text
#   anchor.
...
text <- function(top = NULL, left = NULL, bottom = NULL, right = NULL, 
    text = NULL, stroke = "black", valign = "center", halign = "center", rot = 0, 
    margin = NULL, font = "Arial", italic = F, pointsize = 12, parent = NULL) {
    if ((is.null(left) & is.null(right)) | (!is.null(left) & !is.null(right)) | (is.null(top) & 
        is.null(bottom)) | (!is.null(top) & !is.null(bottom))) {
        print("need one of {left,right} and {top,bottom}")
    }
    
    structure(list(top = top, left = left, bottom = bottom, right = right, text = text, 
        stroke = stroke, valign = valign, halign = halign, rot = rot, margin = margin, 
        font = font, italic = italic, pointsize = pointsize, parent = parent), class = c("cranvas", 
        "text"))
}
...
draw.rect <- function(mark, canvas) {
    ...
    qdrawRect(p = canvas, xleft = xleft, ybottom, xright, ytop, stroke = mark$stroke, 
        fill = mark$fill)
}
...
draw.hbar <- function(mark, canvas) {
    if (is.null(mark$left)) {
        temp <- vector(mode = "numeric", length = max(length(mark$top), length(mark$bottom)) * 
            3)
...
draw.vbar <- function(mark, canvas) {
    if (is.null(mark$top)) {
        temp <- vector(mode = "numeric", length = max(length(mark$left), length(mark$right)) * 
            3)
        for (i in 1:(length(temp)/3)) {
    ...
    if (!is.null(mark$left)) {
        ...
        x <- temp
        # x <- c( mark$parent$limits$height() - mark$right, mark$parent$limits$height()
        #   - mark$right)
        x <- x + mark$parent$limits$left()
    }
...
draw.text <- function(mark, canvas) {
    ...
    qdrawText(canvas, text = mark$text, x = anchor_left, y = anchor_bottom, valign = mark$valign, 
        halign = mark$halign, rot = mark$rot)
}
...
new_plot <- function(width = 600, height = 400, xrange = c(0, 1), yrange = c(0, 
    1)) {
    ...
    self <- structure(list(scene = scene, root = root, limits = limits, size = size), 
        class = "cranvas-plot")
    self
}
#End blank canvas
...
add_layer <- function(parent, mark, keyPressFun = NULL, keyReleaseFun = NULL, 
    mouseDoubleClickFun = NULL, mouseMoveFun = NULL, mousePressFun = NULL, mouseReleaseFun = NULL, 
    wheelFun = NULL, hoverMoveFun = NULL, hoverEnterFun = NULL, hoverLeaveFun = NULL, 
    contextMenuFun = NULL, dragEnterFun = NULL, dragLeaveFun = NULL, dragMoveFun = NULL, 
    dropFun = NULL, focusInFun = NULL, focusOutFun = NULL, sizeHintFun = NULL, row = 0L, 
    col = 0L, userlimits = NULL, geometry = qrect(0, 0, 600, 400), clip = F, colspan = 1L, 
    rowspan = 1L) {
    ...
    layer <- qlayer(parent = parent$root, paintFun = paintFun, keyPressFun = keyPressFun, 
        ...
        wheelFun = wheelFun, hoverMoveFun = hoverMoveFun, hoverEnterFun = hoverEnterFun, 
        hoverLeaveFun = hoverLeaveFun, contextMenuFun = contextMenuFun, dragEnterFun = dragEnterFun, 
        dragLeaveFun = dragLeaveFun, dragMoveFun = dragMoveFun, dropFun = dropFun, 
        focusInFun = focusInFun, focusOutFun = focusOutFun, sizeHintFun = sizeHintFun, 
        clip = clip, limits = limits, row = row, col = col, geometry = geometry, 
...
#--------------------------------
#modify a layer
modify_layer <- function(layerID, parent, show = NULL, enabled = NULL, 
    alpha = NULL, shift_right = NULL, shift_down = NULL) {
