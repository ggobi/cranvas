...
draw_grid <- function(plotObj, dataRanges, row = 0L, col = 0L) {
    ...
    #  bprint(xGridLines)
    #  bprint(yGridLines)
    draw_grid_with_positions(plotObj, dataRanges, xGridLines, yGridLines, row = row, 
        col = col)
...
draw_grid_with_positions <- function(plotObj, dataRanges, horiPos = NULL, 
    vertPos = NULL, minor.horiPos = NULL, minor.vertPos = NULL, row = 0L, col = 0L) {
    
    #background
    plotObj$add_layer(rect(left = dataRanges[1], right = dataRanges[2], bottom = dataRanges[3], 
        top = dataRanges[4], fill = "grey80", stroke = "grey80"), row = row, col = col)
    
    #horizontal
    if (!is.null(vertPos)) {
        plotObj$add_layer(line(left = rep(c(dataRanges[1:2], NA), length(vertPos)), 
            bottom = rep(vertPos, each = 3), stroke = "white"), row = row, col = col)
    ...
    if (!is.null(horiPos)) {
        ...
        plotObj$add_layer(line(left = rep(horiPos, each = 3), bottom = rep(c(dataRanges[3:4], 
            NA), length(horiPos)), stroke = "white"), row = row, col = col)
    ...
    if (!is.null(minor.vertPos)) {
        # change linewidth to smaller width
        plotObj$add_layer(line(left = rep(c(dataRanges[1:2], NA), length(minor.vertPos)), 
            bottom = rep(minor.vertPos, each = 3), stroke = "white", width = 0.1), 
            row = row, col = col)
    ...
    if (!is.null(minor.horiPos)) {
        ...
        # change linewidth to smaller width
        plotObj$add_layer(line(left = rep(minor.horiPos, each = 3), bottom = rep(c(dataRanges[3:4], 
            NA), length(minor.horiPos)), stroke = "white", width = 0.1), row = row, 
            col = col)
    }
...
draw_x_axes_with_labels <- function(plotObj, dataRanges, axisLabels, labelHoriPos, 
    ...
    # draw tick marks
    plotObj$add_layer(line(left = rep(labelHoriPos, each = 3), bottom = rep(c(dataRanges[3], 
        dataRanges[3] - 0.02 * diff(dataRanges[3:4]), NA), length(labelHoriPos)), 
        stroke = "grey30"), row = row, col = col)
    
    # draw labels
    plotObj$add_layer(text(text = axisLabels, left = labelHoriPos, bottom = x_labelpos, 
        stroke = "grey30", valign = "top"), row = row, col = col)
    
    # draw X label
    if (!is.null(name)) {
        plotObj$add_layer(text(text = name, left = x_left[1] + 0.5 * diff(x_left), 
            bottom = dataRanges[3] - 0.07 * diff(dataRanges[3:4]), stroke = "black", 
            valign = "center"), row = row, col = col)
    }
...
draw_y_axes <- function(plotObj, dataRanges, name = NULL, row = 0L, col = 0L) {
    ...
    # yRangeLabels <- c(0, yRangeLabels[yRangeLabels > dataRanges[3] & yRangeLabels
    #   <= dataRanges[4]])
    
    draw_y_axes_with_labels(plotObj, dataRanges, as.character(yRangeLabels), yRangeLabels, 
        name, row, col)
}
draw_y_axes_fun <- function(plotObj, dataRanges, name = NULL) {
    yRangeLabels <- pretty(dataRanges[3:4])
    yRangeLabels <- make_pretty_axes(dataRanges[3:4], dataRanges[3], dataRanges[4])
    draw_y_axes_with_labels_fun(plotObj, dataRanges, yRangeLabels, yRangeLabels, 
        name)
}
...
draw_y_axes_with_labels <- function(plotObj, dataRanges, axisLabels, labelVertPos, 
    ...
    plotObj$add_layer(line(left = rep(c(dataRanges[1] - 0.02 * diff(dataRanges[1:2]), 
        dataRanges[1], NA), length(labelVertPos)), bottom = rep(labelVertPos, each = 3), 
        stroke = "grey30"), row = row, col = col)
    
    
    plotObj$add_layer(text(text = axisLabels, left = y_labelpos, bottom = labelVertPos, 
        stroke = "grey30", halign = "right"), row = row, col = col)
    
    if (!is.null(name)) {
        plotObj$add_layer(text(text = name, left = dataRanges[1] - 0.1 * diff(dataRanges[1:2]), 
            bottom = y_bottom[1] + 0.5 * diff(y_bottom), stroke = "black", halign = "center", 
            rot = 90), row = row, col = col)
    }
...
add_title <- function(plotObj, dataRanges, title) {
    plotObj$add_layer(text(text = title, left = dataRanges[1] + 0.5 * diff(dataRanges[1:2]), 
        bottom = dataRanges[4] + 0.05 * diff(dataRanges[3:4]), stroke = "black", 
        valign = "top"))
}
