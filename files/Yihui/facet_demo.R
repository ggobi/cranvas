...
facet_demo = function(data, facet) {
    ...
    for (i in 1:nl) {
        qlayer(root, function(item, painter) {
            qdrawText(painter, format(pretty(data[, 2])), 9, pretty(data[, 2]), "right", 
                "center")
        }, limits = qrect(c(0, 10), ylim), clip = FALSE, row = i, col = 0)
    }
    qlayer(root, function(item, painter) {
        qdrawText(painter, format(pretty(data[, 1])), pretty(data[, 1]), 9, "center", 
            "top")
    }, limits = qrect(xlim, c(0, 10)), clip = FALSE, row = nl + 1, col = 1)
    ...
    for (i in 1:nl) {
        ...
        # scatter plot
        qlayer(root, function(item, painter) {
            qdrawCircle(painter, df[, 1], df[, 2], 2, stroke = "black", fill = "black")
            qdrawRect(painter, xlim[1], ylim[1], xlim[2], ylim[2], stroke = "black", 
                fill = NA)
        }, limits = qrect(xlim, ylim), clip = FALSE, row = i, col = 1)
        # strip labels
        qlayer(root, function(item, painter) {
