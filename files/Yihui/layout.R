...
titlePainter = function(item, painter) {
    qdrawText(painter, paste(rep("The Useless Title", 5), collapse = " - "), 300, 
        10, "center", "center")
}
...
yaxis = qlayer(figLayer, yAxisPainter, mousePressFun = mouseLocator, limits = qrect(-10, 
    -10, 75, 500), row = 1, col = 0)

plotLayer = qlayer(figLayer, plotPainter, mousePressFun = mouseLocator, 
    limits = qrect(-100, -100, 700, 500), row = 1, col = 1)

xaxis = qlayer(figLayer, xAxisPainter, mousePressFun = mouseLocator, limits = qrect(-10, 
    -10, 700, 95), row = 2, col = 1)
