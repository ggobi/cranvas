...
qmap <- function(data, id = "id", x = "x", y = "y", label = "label", vars, 
    ...
    data.map <- data.frame(id = data[, id], x = data[, x], y = data[, y], label = data[, 
        label])
    
    xr <- range(data.map$x)
    yr <- range(data.map$y)
    # lims <- qrect(xr[1]-0.1*(xr[2]-xr[1]), yr[1]-0.1*(yr[2]-yr[1]),
    #   xr[1]+0.1*(xr[2]-xr[1]),yr[1]+0.1*(yr[2]-yr[1]))
    ...
    map_hover_draw <- function(item, painter, exposed, ...) {
        ...
        # section <- subset(maps_info$data, (y <= top) & (y >= bottom) & (x <= right) &
        #   (x >=left))
        ...
        qdrawText(painter, infostring, .map_queryPos[1], .map_queryPos[2], valign = "top", 
            halign = "left")
        
        # .map_hover_section <<- list(top = section$top, bottom = section$bottom, left
        #   = section$left, right = section$right)
    }
    ...
    map_hover <- function(item, event, ...) {
        ...
        # if ( !((y <= .map_hover_section$top) & (y >= .map_hover_section$bottom) & (x
        #   <= .map_hover_section$right) & (x >= .map_hover_section$left))) {
        qupdate(hoverlayer)
    }
    ...
    hoverlayer = qlayer(scene, map_hover_draw, limits = lims, clip = FALSE, hoverMoveFun = map_hover, 
        hoverLeaveFun = polygon_leave)
