...
gui_xy <- function(qdata = qflea, ...) {
    ...
    # Get the brush color
    bcol <<- brush_attr(qdata, ".brushed.color")
    
    num <- sapply(data, is.numeric)
    
    cat("Starting the tour...\n")
    ...
    update_tour <- function(...) {
        ...
        data <- data.frame(qdata)
        tour <<- create_tour(data, var_selected = svalue(Variables), cat_selected = svalue(Class), 
            axes_location = svalue(dl), tour_type = svalue(TourType), aps = svalue(sl), 
            cur_proj = cur_proj)
        cat("update_tour brush values ", data$.brushed[1], data$.brushed[500], "colours ", 
            tour$colour[1], tour$colour[500], "\n")
    ...
    # ==================Controls==========================
    w <- gwindow("XY tour", visible = FALSE, handler = function(...) {
        pause(TRUE)
        view$close()
    })
    vbox <- glayout(cont = w)
    ...
    vbox[2, 1] <- Variables <- gcheckboxgroup(names(data[num]), checked = TRUE, horizontal = FALSE, 
        handler = update_tour)
    ...
    tour_types <- c("Grand", "Little", "Guided(holes)", "Guided(cm)", "Guided(lda_pp)", 
        "Local")
    ...
    aes_box[1, 2, expand = TRUE] <- sl <- gslider(from = 0, to = 5, by = 0.1, value = 1)
    aes_box[2, 1, anchor = c(1, -1)] <- "Transparency"
    aes_box[2, 2, expand = TRUE] <- sl_alpha <- gslider(from = 0.01, to = 1, by = 0.01, 
        value = 1)
    aes_box[3, 1, anchor = c(1, -1)] <- "Size"
    aes_box[3, 2, expand = TRUE] <- sl_size <- gslider(from = 0, to = 8, by = 0.5, 
        value = 2)
    ...
    chk_pause <- gcheckbox("Pause", handler = function(h, ...) pause(svalue(h$obj)))
    vbox[1, 3] <- chk_pause
    
    buttonGroup <- ggroup(horizontal = F, cont = vbox)
    glabel("Optimise for:", cont = buttonGroup)
    gradio(c("Quality", "Speed"), cont = buttonGroup, handler = function(ev, ...) {
    ...
    scene <- qscene()
    root <- qlayer(scene, geometry = qrect(0, 0, 600, 600))
    #  root <- qlayer(scene)
...
create_tour <- function(data, var_selected, cat_selected, axes_location, 
    tour_type, aps, cur_proj) {
    ...
    #  }
    cat("create_tour brush values ", data$.brushed[1], data$.brushed[500], "colours ", 
        data$.color[1], col[1], data$.color[500], col[500], "\n")
    
    # Work out which type of tour to use
    tour <- switch(tour_type, Grand = grand_tour(), Little = little_tour(), `Guided(holes)` = guided_tour(holes), 
        `Guided(cm)` = guided_tour(cm), `Guided(lda_pp)` = guided_tour(lda_pp(data[, 
            cat_selected])), Local = local_tour(cur_proj))
    ...
    list(data = rescale(sel), tour_path = tour, colour = col, aps = aps)
} 
