...
zoom_update <- function() {
    qlimits(points) <- qrect(c(pos[1] - rng[1]/1.4^zoom_level, pos[1] + rng[1]/1.4^zoom_level), 
...
points <- qlayer(scene, scatterplot, mouseDoubleClickFun = mouse_zoom, 
    keyPressFun = handle_keys)
zoom_update()
