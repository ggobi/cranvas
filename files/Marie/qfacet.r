...
qfacet <- function(data, xyformula = NULL, facetformula = NULL, nrow = NULL, 
    ncol = NULL) {
    #############################
    ...
    draw_facetlab <- function(item, painter, exposed) {
        qdrawRect(painter, xleft = 0, ybottom = 0, xright = 1, ytop = 1, fill = "grey80", 
            stroke = "grey80")
    }
    ...
    back <- function(item, painter, exposed) {
        ...
        # qdrawRect(painter, xleft = 0, ybottom = 0, xright = 1, ytop = 1, fill =
        #   'grey80', stroke = 'grey80')
        #qdrawLine(painter, x = rep(c(dataRanges[1:2],NA), length(sy)), y = rep(sy,
        #   each = 3), stroke = 'white')
        # qdrawLine( painter, x = rep(sx, each=3), y= rep(c(dataRanges[3:4],NA),
        #   length(sx)), stroke = 'white' )
    }
    
    
    dl <- function(row, col) {
        temp <- df1[subset(df1, select = .facetX) == sort(unique(subset(df1, select = .facetX)[[1]]))[col - 
            1] & subset(df1, select = .facetY) == sort(unique(subset(df1, select = .facetY)[[1]]))[row - 
            1], ]
        ...
        dlpaint <- function(item, painter, exposed) {
            radius <- .radius
            if (!is.na(x[1])) {
                qdrawCircle(painter, x = x, y = y, r = radius, fill = unlist(fill), 
                  stroke = unlist(stroke))
            }
        }
    ...
    ylab <- add_layer(parent = plot1, mark = text(text = .levelY, left = 0.5, bottom = 0.5, 
        parent = plot1, rot = 90), col = 0, row = 2, rowspan = as.integer(grid[2]))
    xlab <- add_layer(parent = plot1, mark = text(text = .levelX, left = 0.5, bottom = 0.5, 
        parent = plot1), col = 2, row = grid[2] + 3, colspan = as.integer(grid[1]))
    
    #note: this plot is built with the idea that (eventually) user will be able to
    #   change x and y selections, but not faceting
    #axes labels
    ...
    # facet labels
    for (i in 1:grid[1]) {
        add_layer(plot1, text(text = tolower(get_facetlabel(.facetX, ncol, grid[1])[i]), 
            left = 0.5, bottom = 1, valign = "top", parent = plot1), col = i + 1, 
            row = 1)
    }
    for (i in 1:grid[2]) {
        add_layer(plot1, text(text = tolower(get_facetlabel(.facetY, nrow, grid[2])[i]), 
            left = 0.5, bottom = 0.5, rot = -90, parent = plot1), col = grid[1] + 
            2, row = i + 1)
    }
    
    for (i in 1:grid[2]) {
        for (j in 1:grid[1]) {
            assign(paste("back", i, j, sep = "_"), add_layer(plot1, mark = back, 
                row = i + 1, col = j + 1, colspan = 1, rowspan = 1), pos = 1)
        }
    }
    ...
    for (i in 0:(grid[1] + 3)) {
        for (j in 0:(grid[2] + 3)) {
            add_layer(parent = plot1, mark = rect(left = 0, bottom = 0, height = 1, 
                width = 1, parent = plot1, stroke = "white", fill = NULL), col = i, 
                row = j)
    ...
    for (i in 1:grid[2]) {
        for (j in 1:grid[1]) {
            qlayer(parent = plot1$root, paintFun = dl(row = i + 1, col = j + 1), 
                row = i + 1, col = j + 1, limits = qrect(c(0, 1), c(0, 1)))
            qlayer(parent = plot1$root, paintFun = bl(row = i + 1, col = j + 1), 
                limits = qrect(c(0, 1), c(0, 1)))
        }
    }
    
    brushlayer_2_2 <- qlayer(parent = plot1$root, paintFun = bl(row = 4, col = 3), 
        limits = qrect(c(0, 1), c(0, 1)))
