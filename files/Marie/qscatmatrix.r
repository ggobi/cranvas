...
qscatmatrix <- function(data) {
    ...
    #print('------')
    #print(paste('odata attribute at import:', brush_attr(odata,
    #   '.brushed.color')))
    #print('-----1')
        ...
        dlpaint <- function(item, painter, exposed) {
            ...
            if (!is.na(x[1])) {
                if (colnames(odata)[col] != colnames(odata)[row]) {
                  ...
                  qdrawText(painter, text = colnames(odata)[row], x = 0.5, y = 0.5)
                }
            }
        }
    }
    ...
    add_layer(parent = plot1, mark = draw_facetlab, row = 2, col = nrowscols + 2, 
        rowspan = nrowscols)
    
    # facet labels
    for (i in 1:nrowscols) {
        add_layer(plot1, text(text = names(odata)[i], left = 0.5, bottom = 1, valign = "top", 
            parent = plot1, pointsize = 10), col = i + 1, row = 1)
    }
    
    for (i in 1:nrowscols) {
        add_layer(plot1, text(text = names(odata)[i], left = 0.75, bottom = 0.5, 
            rot = -90, valign = "top", parent = plot1, pointsize = 9), col = nrowscols + 
            2, row = i + 1)
    }
    ...
    for (i in 1:nrowscols) {
        for (j in 1:nrowscols) {
            add_layer(plot1, mark = back, row = i + 1, col = j + 1, colspan = 1, 
                rowspan = 1)
        }
    }
    ...
    for (i in 1:nrowscols) {
        for (j in 1:nrowscols) {
            qlayer(parent = plot1$root, paintFun = dl(row = i, col = j), row = i + 
                1, col = j + 1, limits = qrect(c(0, 1), c(0, 1)))
        }
    }
    ...
    for (i in 1:nrowscols) {
        for (j in 1:nrowscols) {
            blayer <- qlayer(parent = plot1$root, paintFun = bl(row = i, col = j), 
