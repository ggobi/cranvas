...
draw_grid_with_positions <- function(plotObj, border_x, border_y, horiPos = NULL, 
    vertPos = NULL, minor.horiPos = NULL, minor.vertPos = NULL, row = 0, col = 0, 
    geometry = qrect(0, 0, 600, 400)) {
    
    #background: one grey rectangle
    add_layer(parent = plotObj, mark = rect(left = border_x, right = border_x * 0.25, 
        bottom = border_y, top = border_y * 0.75, fill = "grey90", stroke = "grey90", 
        parent = plotObj), row = row, col = col, geometry = geometry)
    
    #vertical major grid lines
    if (!is.null(vertPos)) {
        add_layer(parent = plotObj, mark = vbar(left = vertPos, stroke = "white", 
            parent = plotObj, width = 2), row = row, col = col, geometry = geometry)
    ...
    if (!is.null(horiPos)) {
        add_layer(parent = plotObj, mark = hbar(bottom = horiPos, stroke = "white", 
            parent = plotObj, width = 2), row = row, col = col, geometry = geometry)
    ...
    if (!is.null(minor.horiPos)) {
        add_layer(parent = plotObj, mark = hbar(bottom = minor.horiPos, stroke = "white", 
            width = 1, parent = plotObj), row = row, col = col, geometry = geometry)
        
    }
    
    #minor vertical
    if (!is.null(minor.vertPos)) {
        add_layer(parent = plotObj, mark = vbar(left = minor.vertPos, stroke = "white", 
            width = 1, parent = plotObj), row = row, col = col, geometry = geometry)
    }
}
...
#' @keywords hplot
draw_x_axes_with_labels <- function(plotObj, dataRanges, axisLabels, labelHoriPos, 
    name = NULL, row = 0L, col = 0L, xspan = NULL, yspan = NULL, geometry = qrect(0, 
        0, 600, 400)) {
    #  X label
    ...
    add_layer(parent = plotObj, mark = vbar(left = labelHoriPos - plotObj$limits$left(), 
        bottom = (0.1 * plotObj$limits$height()), top = 0.78 * plotObj$limits$height(), 
        stroke = "grey30", parent = plotObj), row = row, col = col, geometry = geometry)
    
    #draw data labels, x axis
    add_layer(parent = plotObj, mark = text(text = axisLabels, left = labelHoriPos - 
        plotObj$limits$left(), bottom = 0.1 * yspan, stroke = "grey30", valign = "top", 
        parent = plotObj), row = row, col = col, geometry = geometry)
    
    #draw x axis label
    if (!is.null(name)) {
        add_layer(parent = plotObj, mark = text(text = name, left = x_left[1] + 0.5 * 
            diff(x_left), bottom = dataRanges[3] - 0.13 * diff(dataRanges[3:4]), 
            stroke = "black", valign = "center", parent = plotObj), row = row, col = col, 
            geometry = geometry)
    }
}
#' draw y axes
...
#' @author Barret Schloerke \email{bigbear@@iastate.edu}
#' @keywords hplot
draw_y_axes_with_labels <- function(plotObj, dataRanges, axisLabels, labelVertPos, 
    name = NULL, row = 0L, col = 0L, xspan = NULL, yspan = NULL, geometry = qrect(0, 
        0, 600, 400)) {
    # Y label
    ...
    # draw axis
    add_layer(parent = plotObj, mark = hbar(left = 0.15 * xspan, right = 0.86 * xspan, 
        bottom = labelVertPos - plotObj$limits$top(), stroke = "grey30", parent = plotObj), 
        row = row, col = col, geometry = geometry)
    # draw labels
    add_layer(parent = plotObj, mark = text(text = axisLabels, right = 0.87 * xspan, 
        bottom = labelVertPos - plotObj$limits$top(), stroke = "grey30", halign = "right", 
        valign = "center", parent = plotObj), row = row, col = col, geometry = geometry)
    
    if (!is.null(name)) {
        add_layer(parent = plotObj, mark = text(text = name, left = dataRanges[1] - 
            0.18 * diff(dataRanges[1:2]), bottom = y_bottom[1] + 0.5 * diff(y_bottom), 
            stroke = "black", valign = "center", rot = 90, parent = plotObj), row = row, 
            col = col, geometry = geometry)
    }
}

draw_x_facet_labels <- function(plotObj, border_x, yspan, xspan, row = 0L, 
    col = 0L, geometry = qrect(0, 0, 600, 400), name) {
    add_layer(parent = plotObj, mark = rect(left = border_x, right = border_x * 0.25, 
        bottom = 0.93 * yspan, top = 0, fill = "grey80", stroke = "grey80", parent = plotObj), 
        row = row, col = col, geometry = geometry)
    add_layer(parent = plotObj, mark = text(text = name, left = 0.5 * xspan, top = 0.04 * 
        yspan, stroke = "black", valign = "center", parent = plotObj), row = row, 
        col = col, geometry = geometry)
    
}

draw_y_facet_labels <- function(plotObj, border_y, xspan, yspan, row = 0L, 
    col = 0L, geometry = qrect(0, 0, 600, 400), name) {
    add_layer(parent = plotObj, mark = rect(left = 0.95 * xspan, right = 0, bottom = border_y, 
        top = border_y * 0.75, fill = "grey80", stroke = "grey80", parent = plotObj), 
        row = row, col = col, geometry = geometry)
    add_layer(parent = plotObj, mark = text(text = name, left = 0.94 * xspan, bottom = 0.5 * 
        yspan, stroke = "black", rot = -90, valign = "bottom", parent = plotObj), 
        row = row, col = col, geometry = geometry)
}
...
draw_grid_with_positions_fun <- function(plotObj, dataRanges, horiPos = NULL, 
    ...
    if (!is.null(vertPos)) {
        ...
        qdrawLine(plotObj, x = rep(c(dataRanges[1:2], NA), length(vertPos)), y = rep(vertPos, 
            each = 3), stroke = "white")
...
draw_x_axes_fun <- function(plotObj, dataRanges, name = NULL) {
    ...
    draw_x_axes_with_labels_fun(plotObj, dataRanges, xRangeLabels, xRangeLabels, 
        name)
}
