...
#	if (!is.mutaframe(data)) data <- qmutaframe(data)
  ...
  .brush.attr = attr(data, '.brush.attr')
	...
	# update .data.summary$mvBrushed
		.data.summary$mvBrushed <<- ldply(df.data[,.data.summary$Names], function(x) return(sum(is.na(x) & data$.brushed)))$V1
		.data.summary$valBrushed <<- ldply(df.data[,.data.summary$Names], function(x) return(sum(!is.na(x) & data$.brushed)))$V1
	}
  ...
  brushing_draw <- function(item, painter, exposed, ...) {
    if (!is.null(.endBrush)) {
      drawBrush(item, painter, exposed)
		...
		n <- nrow(data)
  	for (i in 1:p) { 		
			qdrawRect(painter, left, bottom, (right*.data.summary$valBrushed[i])/n, top, 
