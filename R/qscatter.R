...
#               2, byrow = TRUE))
  ...
  query_draw <- function(layer, painter) {
    ...
    qdrawRect(painter, xpos, ypos, xpos + ifelse(hflag, 1, -1) * bgwidth,
      ypos + ifelse(vflag, -1, 1) * bgheight, stroke = rgb(1, 1, 1),
      fill = rgb(1, 1, 1, 0.9))
...
#  root$setGeometry(qrect(0, 0, xWidth, yWidth))
  ...
  xaxis <- qlayer(parent=root, paintFun = xaxis,
    limits = qrect(dataRanges[1:2], c(0, 1)), cache=cache, row=2, col=1,
    clip=FALSE)
  yaxis <- qlayer(parent=root, paintFun = yaxis,
    limits = qrect(c(0, 1), dataRanges[3:4]), cache=cache, row=1, col=0,
    clip=FALSE)
  ...
  datalayer <- qlayer(parent = root, paintFun = scatter.all,
    ...
    wheelFun = handle_wheel_event, limits = lims, cache=cache, row=1,
      col=1, clip=FALSE)
