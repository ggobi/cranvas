...
qts <- function(time, y, data) {
    ...
    brush_mouse_release <- function(layer, event) {
        ...
        idx <- tmpdata$time > min(.bstart[1], .bend[1]) & data[, 
            cy] > min(.bstart[2], .bend[2]) & tmpdata$time < 
            max(.bstart[1], .bend[1]) & data[, cy] < max(.bstart[2], 
            .bend[2])
        selected(data) <- idx
        if (length(idx)) 
            qupdate(brush_layer)
    ...
    key_press <- function(layer, event) {
        ...
        main_layer$setLimits(qrect(range(tmpdata$time), range(data[, 
            cy])))
        brush_layer$setLimits(qrect(range(tmpdata$time), range(data[, 
            cy])))
        qupdate(main_layer)
        qupdate(xaxis)
    }
    main_draw <- function(layer, painter) {
        ...
        for (i in 1:max(tmpdata$zoomgroup)) {
            qdrawCircle(painter, tmpdata[tmpdata$zoomgroup == 
                i, 1], data[tmpdata$zoomgroup == i, cy], 2, fill = color[i], 
                stroke = color[i])
            qdrawLine(painter, tmpdata[tmpdata$zoomgroup == i, 
                1], data[tmpdata$zoomgroup == i, cy], stroke = color[i])
        }
    }
    brush_draw <- function(layer, painter) {
        ...
        if (any(idx)) {
            txt <- paste("Point ", rownames(data)[idx], "\nTime: ", 
                data[idx, time], "\nValue: ", data[idx, cy], 
                sep = "")
            qdrawText(painter, txt, tmpdata[idx, 1], data[idx, 
                cy])
        }
    }
    scene <- qscene()
    ...
    xaxis <- qaxis(root_layer, data = tmpdata$time, side = 1, 
        limits = range(tmpdata$time), row = 2, col = 1)
    ...
    main_layer <- qlayer(root_layer, paintFun = main_draw, mousePressFun = brush_mouse_press, 
        mouseReleaseFun = brush_mouse_release, wheelFun = mouse_wheel, 
        keyPressFun = key_press, limits = qrect(range(tmpdata$time), 
